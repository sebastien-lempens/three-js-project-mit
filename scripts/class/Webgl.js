...
export default class Webgl {
	...
	async initThree() {
		...
		if (this.app.xr) {
			...
			this.app.scene.position.set(0, 0, 0)
			this.camera.position.set(0, 0, 0)
			this.app.scene.scale.addScalar(100)
	...
	initLights() {
		...
		this.app.scene.add(this.ambiantLight)
		/**
		 * Spot Light
		 */
		...
		this.app.scene.add(this.pointLight2)
		...
		this.app.scene.add(this.pointLight2helper)
	}
	async initWorld() {
	...
	async initEffects() {
		const { default: Effects } = await import('./Effects.js')
		...
		const renderPass = new RenderPass(this.app.scene, this.camera)
	...
	update() {
		...
		if (this.assetsLoadedProgress === 0 && this.mainTitleEl.nextSibling) {
			this.mainTitleEl.nextSibling.remove()
			this.assetsProgressing = false
		}
	}
	resize() {
